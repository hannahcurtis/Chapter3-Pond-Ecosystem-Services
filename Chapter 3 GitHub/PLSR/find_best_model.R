...
run_model <- function(X, Y, df, ncomp=3) {
  set.seed(0)
...
compute_vi <- function(pls_model, ncomp=3) {
...
compute_aic <- function(model, ncomp) {
  ...
  AIC <- AICpls(ncomp, model$residuals)
  num_predictors <- ncol(model.matrix(model)) -1  # Includes intercept
  num_params <- num_predictors + model$ncomp
  return(AIC + (2*num_predictors*(num_predictors + 1))/(nrow(df)-num_predictors-1))
}

compute_bic <- function(model, ncomp) {
  n <- dim(model$residuals)[1]
  #return(n * log(sum(model$residuals[,,ncomp]^2)/n) + ncomp * log(n))
  num_predictors <- ncol(model.matrix(model))   # Includes intercept
  num_params <- num_predictors + model$ncomp
  return(n * log(sum(model$residuals[,,ncomp]^2)/n) + num_params * log(n))
}

get_ncomp_for_min_ic <- function(model, ic_func) {
  min_ic <- 1000000
  min_ic_ncomp <- -1
...
get_blank_df <- function() {
  return(data.frame(
    max_q_score=numeric(), min_aic=numeric(), min_bic=numeric(),
    nvar=numeric(), ncomp=numeric(), 
    X=character(), Y=character(), 
    last_removed=character(), least_vi=character(), least_vi_val=numeric())
  )
}
...
eval_vars <- function(X, Y, df, ncomp, to_remove=NULL, method="q-score") {
  ...
  if (method == "aic") {
    best_ncomp <- get_ncomp_for_min_ic(pls_model, ic_func=compute_aic)
  } else if (method == "bic") {
    best_ncomp <- get_ncomp_for_min_ic(pls_model, ic_func=compute_bic)
  } else if (method == "q-score") {
    best_ncomp <- get_ncomp_for_max_qscore(q_score)
  } 
  
  min_aic <- compute_aic(pls_model, best_ncomp)
  min_bic <- compute_bic(pls_model, best_ncomp)
  ...
  return(data.frame(
    max_q_score=max_q_score, min_aic=min_aic, min_bic=min_bic,
    nvar=length(X), ncomp=best_ncomp, 
    X=paste(X, collapse="+"), Y=paste(Y, collapse="+"), 
    last_removed=to_remove, least_vi=vi$Variable[1], least_vi_val=vi$Importance[1]
  ))
}
...
iter_fun <- function(X, Y, df, ncomp, inner_func, comp_func, method) {
  ...
  for (to_remove in X) {
    res_df <- eval_vars(X, Y, df, min(ncomp, length(X)-1), to_remove, method)
...
vi_bic_objective <- function(X,Y,df,ncomp) {
  res_df <- eval_vars(X, Y, df, min(ncomp, length(X)-2), NULL, "bic")
  res_df <- eval_vars(X, Y, df, min(ncomp, length(X)-2), res_df$least_vi, "bic")
  return(res_df)  
}
...
bic_objective <- function(X,Y,df,ncomp) {
  return(iter_fun(X, Y, df, ncomp, function(x) x$min_aic, which.min, "bic"))
}

get_best_vars <- function(X, Y, df, ncomp=3, objective="q-score") {
  ...
  else if (objective == "vi_bic") { obj_fun <- vi_bic_objective }
  else if (objective == "aic") { obj_fun <- aic_objective }
  else if (objective == "bic") { obj_fun <- bic_objective }
  else { stop("objective must be 'q-score', 'aic', 'bic', vi', or 'vi_aic' or 'vi_bic'") }
...
get_best_vars_both <- function(X, Y, df, ncomp=3)
{
...
get_best_vars_generic <- function(X, Y, df, ncomp=3, reduce_model="vi", overall_obj="q-score")
{
  ...
  } else if (overall_obj == "bic") {
    best_1 <- results_1[which.min(results_1$min_bic),]
    best_2 <- results_2[which.min(results_2$min_bic),]
  } else {
    stop("overall_obj must be 'q-score' or 'aic' or 'bic'")
  }
  ...
  if (overall_obj == "q-score") {
    ...
    best_both <- best_model_reduce[which.min(best_model_reduce$min_bic),]
  } else {
    stop("overall_obj must be 'q-score' or 'aic' or 'bic'")
  }
  ...
  while (length(x_1) > 0) {
    ...
    if (overall_obj == "q-score") {
      ...
      best_row <- sub_model_results[which.min(sub_model_results$min_bic),]
    } else {
      stop("overall_obj must be 'q-score' or 'aic' or 'bic'")
    }
    additive_model_results <- rbind(additive_model_results, best_row)
  ...
  if (overall_obj == "q-score") {
    ...
    best_row <- additive_model_results[which.min(additive_model_results$min_bic),]
  } else {
    stop("overall_obj must be 'q-score' or 'aic'")
  }
  print(c("max q-score from additive model:", best_row$max_q_score))
  print(c("min aic from additive model:", best_row$min_aic))
  print(c("min bic from additive model:", best_row$min_bic))
  print(c("additive model vars unique from",reduce_model,"model:", 
          paste(setdiff(strsplit(best_row$X, "\\+")[[1]], x_2), collapse="+")))
